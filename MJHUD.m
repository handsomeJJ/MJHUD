...
+ (void)showInfo:(NSString *)message image:(UIImage *)image{
    ...
    hud.imgSuccess.image = image == nil ? hud.infoImage : image;
...
- (void)layoutSubviews {
    ...
    self.hudView.frame = CGRectMake(hudX, hudY, hudWH, hudWH);
    
    CGFloat hudW = self.hudView.width;
    CGFloat hudH = self.hudView.height;
    ...
    if(![@"" isEqualToString:self.message]) {
        ...
        if (size.height > (MJHUDMessageHeight)) {
            self.hudView.height = CGRectGetMaxY(self.lblMessage.frame) + MJHUDMargin;
            self.hudView.center = self.overlayView.center;
        }
    }
}
